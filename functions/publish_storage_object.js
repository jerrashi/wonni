...
// download-token URL — a different host than every server-side upload
// (AliExpress/Weverse import, image split) already uses. The Mercari cross-post
// extension's background service worker can only bypass CORS for
// storage.googleapis.com (per manifest host_permissions), so a
// firebasestorage.googleapis.com image silently fails to fetch and gets dropped
// from the listing. This flips a just-uploaded object to public and returns its
// canonical storage.googleapis.com URL, so every image URL in the app — client
// or server uploaded — uses the same format.
