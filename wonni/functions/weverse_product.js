...
function mapWeverseVariantsToOptions(rawVariants) {
  ...
  const variants = parsed.map((v, i) => {
    ...
    return {
      ...
      // Every variant starts active with 0 stock (maxOrderQuantity is a
      // per-order purchase limit, not actual inventory) — the user fills in
      // real stock per variant afterward rather than everything defaulting to 1.
      quantity: 0,
